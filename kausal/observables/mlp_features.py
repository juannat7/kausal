...
class MLPFeatures(BaseObservables):
    """
    Generate latent vector z for input data X, with multi-layer perceptron.
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            *[
                nn.Sequential(
                    nn.Linear(forward_channels[i], forward_channels[i + 1], bias=False),
        ...
        self.decoder = nn.Sequential(
            *[
                nn.Sequential(
                    nn.Linear(reverse_channels[i], reverse_channels[i + 1], bias=False),
                    self.activation if i < len(hidden_channels) - 1 else nn.Identity()
                )
                for i in range(len(hidden_channels))
            ]
    ...
    def forward(
        ...
        Assumes input of shape (C, T).
        """
        assert X.ndim == 2, 'MLP assumes input of shape (C, T)!'
        C, T = X.shape
        
        X = X.to(self.device)
        X = X.permute(1, 0) # to shape (T, C)
        
        X = self.encoder(X)
        
        X = X.transpose(1, 0) # to shape (M, T)
        X = X.to('cpu')
        return X
